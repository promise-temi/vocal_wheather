...
app.get('/activate-vocal-command')
def activate_vocal_command():
    try:
        #Réccupération du message grace a la commande vocal dans une variable
        message = recognize_from_microphone()
        print(message)
        return jsonify({'message': f'Message enregistré : \'{message}\''}), 200
    except:
        return jsonify({'message': 'Une erreur s\'est produite'}), 404
